...
import { TafeClient } from "./TafeClient";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const t = await getTranslations({ locale, namespace: "tafeMetadata" });
    return {
        title: t("title"),
...
export default async function TafePage({ params }: Props) {
    const { locale } = await params;
    ...
    const t = await getTranslations({ locale, namespace: "schoolPages.tafe" });
